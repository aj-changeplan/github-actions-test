...
echo "Restarting PM2";
echo "==================";
sshpass -p$HOST_PASSWORD ssh -o StrictHostKeyChecking=no -o ProxyCommand="sshpass -p$PROXY_HOST_PASSWORD ssh -o StrictHostKeyChecking=no -W %h:%p $USER@$PROXY_HOST" $USER@$SERVER_HOST "source ~/.bash_profile && pm2 restart $PM2_NAME"
